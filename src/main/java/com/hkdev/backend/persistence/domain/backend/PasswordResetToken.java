...
import com.hkdev.backend.persistence.converters.LocalDateTimeAttributeConverter;
...
public class PasswordResetToken implements Serializable {
    ...
    public PasswordResetToken(String token, User user, LocalDateTime creationDateTime, int expirationInMinutes) {
        if(token == null || user == null || creationDateTime == null) {
            throw new IllegalArgumentException("Token, user or creationDateTime can't be null!");
        }

        if(expirationInMinutes == 0) {
            LOGGER.warn("The token expiration length is zero. Assigning the default value {}", DEFAULT_TOKEN_LENGTH_IN_MINUTES);
